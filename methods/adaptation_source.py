...
def train_sam(
    ...
    """Source-only baseline: inference without test-time adaptation."""
    ...
    for epoch in range(num_epochs):
        for iter, data in enumerate(all_dataloader):
            ...
            with torch.no_grad():
                _, pred_masks, ious, _ = anchor_model(images_test, prompts)
            ...
            iou_scores.update(batch_iou, num_images)
            p_iou_scores.update(iou.mean().item(), num_images)
...
def load_config(cfg_path: str, args: argparse.Namespace) -> Box:
    ...
    cfg.out_dir = "output/source-only-tta/" + args.dataset
    return cfg
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Source-only baseline for SAM (no test-time adaptation)")
